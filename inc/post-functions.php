...
if ( ! function_exists( 'r1_post_content' ) ) {
	...
	function r1_post_content( string $button_title = '', int $max_length = -1 ): string {
		...
		$button_title = empty( $button_title ) ? esc_html__( 'Read More', 'dtc' ) : $button_title;
		...
		ob_start();
			...
			wp_link_pages(
				array(
					'before'      => '<div class="paging-navigation in-post"><div class="pagination"><span class="page-links-title">' . esc_html__( 'Pages:', 'dtc' ) . '</span>',
					'after'       => '</div></div>',
					'link_before' => '<span>',
					'link_after'  => '</span>',
				)
